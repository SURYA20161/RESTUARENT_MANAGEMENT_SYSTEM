...
export const createOrder = async (req, res) => {
  ...
  try {
    const { products, totalAmount, address, phoneNumbers, paymentMethod } = req.body;
...
`.trim();
    ...
    const productDetails = populatedOrder.products.filter(item => item.product);
    ...
    if (userDetails) {
      ...
      console.error("User details not found, skipping email");
    }
...
export const getAllOrders = async (req, res) => {
  try {
    const orders = await Order.find()
      ...
      .sort({ _id: -1 }); // Sort by _id descending to show recent orders first (since _id contains timestamp)
