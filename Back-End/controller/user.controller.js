...
export const signup = async (req, res) => {
  try {
    ...
    // Send the verification email (commented out for localhost)
    // try {
    //   await sendMail(
    //     email,
    //     "Email Verification",
...
export const getUserProfile = async (req, res) => {
  try {
    const user = await User.findById(req.user._id).select('-password -resetPasswordToken -resetPasswordExpiresAt -verificationToken -verificationTokenExpiresAt');
...
export const getUserOrders = async (req, res) => {
  try {
    const Order = (await import('../model/order.model.js')).default;
