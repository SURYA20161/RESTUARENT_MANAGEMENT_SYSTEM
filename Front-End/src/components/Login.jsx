...
function Login() {
  ...
  // OAuth callback is now handled in App.jsx
      ...
      if (response.data.token) {
        ...
        setTimeout(() => {
          ...
          if (user && user.role === 'admin') {
            window.location.href = `http://localhost:5174/admin-dashboard?token=${response.data.token}`;
          } else {
            navigate("/profile");
          }
        }, 500);
      ...
      >
        ...
        >
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            {/* Google OAuth Button */}
            <button
              className="w-full py-3 px-6 bg-white hover:bg-gray-100 border border-gray-300 rounded-lg shadow-md text-gray-700 font-semibold transition duration-200 flex items-center justify-center"
              type="button"
